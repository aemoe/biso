...
const LaunchpadDetails = () => {
  ...
  const mint = async (type: number) => {
    ...
    if (new Date().getTime() < 16871796000 * 1000 && type == 1) {
      toast.warning("The Whitelist sale round has yet to begin", toastConfig);
      return;
    }

    if (new Date().getTime() > 16871796000 * 1000 + 15 * 60 * 60 * 1000 && type == 1) {
      toast.warning("The Whitelist sale round has end", toastConfig);
      return;
    }

    if (new Date().getTime() < 16872444000 * 1000 && type == 2) {
      toast.warning("The Public sale round has yet to begin", toastConfig);
      return;
    }

    if (
      new Date().getTime() > 16872444000 * 1000 + 30 * 60 * 60 * 1000 &&
      type == 2
    ) {
      toast.warning("The Whitelist sale round has end", toastConfig);
      return;
            ...
            <div className={styles.card + " " + styles.item}>
              <div className={styles.title}>
                ...
                <div className={styles.deadline}>
                  {new Date().getTime() < 168717960 * 1000 ? (
                    <Timer
                      formatValue={(value) =>
                        `${value < 10 ? `0${value}` : value} `
                      }
                      initialTime={
                        new Date(168717960 * 1000).getTime() -
                        new Date().getTime()
                      }
                      lastUnit="d"
                      direction="backward"
                ...
                <div className={styles.deadline}>
                  {new Date().getTime() < 16872444 * 1000 ? (
                    ...
                    <Timer
                      ...
                      initialTime={
                        new Date(168735240 * 1000).getTime() -
                        new Date().getTime()
                      }
                      lastUnit="d"
                      direction="backward"
